...
class _MonkeyPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final eating = monkey.phase == MonkeyPhase.eating;
    final clapping = monkey.phase == MonkeyPhase.clapping;
    ...
    final reach = Curves.easeOutBack.transform(monkey.reachProgress.clamp(0.0, 1.0));
    // 0..1 beat used for the celebratory clap and hop.
    final clap = clapping ? math.sin(monkey.actionTimer * 14).abs() : 0.0;
    // Chewing rhythm while eating.
    final chew = eating ? (math.sin(eat * math.pi * 8) + 1) / 2 : 0.0;

    final breathe = math.sin(monkey.animPhase * 2) * 1.6;
    final hop = -clap * 7 - reach * 4;
    final tailWag = math.sin(monkey.tailWag * (clapping ? 1.8 : 1)) * (8 + clap * 8);
    ...
    _drawTail(canvas, cx, cy, tailWag);
    ...
    _drawHead(canvas, cx, cy, sad, eat, reach: reach, chew: chew, happy: clapping);
  ...
  void _drawHead(
    ...
    canvas.translate(
      cx + monkey.headShake * 18,
      cy - 12 - reach * 3 + chew * 1.5,
    );
    if (happy) {
      canvas.rotate(math.sin(monkey.actionTimer * 7) * 0.06);
    }
    final droop = -monkey.earDroop * 6;
  ...
  void _drawMouth(Canvas canvas, double sad, double eat, double chew) {
    ...
    if (monkey.phase == MonkeyPhase.eating) {
    ...
    if (monkey.phase == MonkeyPhase.reaching || monkey.phase == MonkeyPhase.catching) {
